Shader "Custom/StarGateNoiseChange2"
{
    // This shader variation changes the way color is applied to the new Cellnoise 
    Properties
    {
        ...
        _CellSize ("Cell Size", Range(0.1, 40)) = 1             // Additional Cell Shader, cell size
        _NoiseScale ("Noise Scale", Range(0.1, 40)) = 1         // General Noise Scale
               ...
               //noiseColor.rgb *= _finalColor.rgb;
                ...
                for (y = -1; y <= 1; y++)
                {
                    for (x = -1; x <= 1; x++)
                    {
                        ...
                        noiseColor.rgb = lerp(_color1.rgb, _color2.rgb, noise);
