Shader "Custom/StarGateExperimentNormal"
{
    // This shader is the Warp Experiment shaders stripped off of their new functions for easy comparison to what the base shader does.
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Random Number Hash Function for Noise
            float hash(float3 p)
            {
            ...
            // Cell Noise generating function
            ...
            {
                // UV displacement based on the warpsize 
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // Generate noise lines with warp_tex
                for (y = -1; y <= 1; y++)
                {
                    for (x = -1; x <= 1; x++)
                    {
                ...
                finalcol.rgb = (clamp(finalcol.rgb - finalFade , 0, 1) * noiseColor.rgb) * emissionStrengthfar;
