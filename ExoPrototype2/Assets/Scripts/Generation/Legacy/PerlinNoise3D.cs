...
public class PerlinNoise3D : MonoBehaviour
{
    ...
    [SerializeField] public float perlinScale;
    ...
    public float PerlinNoise(Vector3 coordinates)
    {
        //Summary
        ...
        //Fanny: Added PerlinScale
        return Mathf.Lerp(y1, y2, w) * perlinScale;
    }
        ...
        for (int x = 0; x < volumeVector.x; x++)
        {
            for (int y = 0; y < volumeVector.y; y++)
            {
                for (int z = 0; z < volumeVector.z; z++)
                {
                    ...
                    float perlinNoiseValue = PerlinNoise(newPosition);
